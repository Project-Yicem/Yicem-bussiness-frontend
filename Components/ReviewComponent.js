...
const ReviewComponent = ({ reviews }) => {
  ...
  const calculateYearlyData = () => {
    ...
    for (let i = 11; i >= 0; i--) {
        ...
        const monthName = new Intl.DateTimeFormat('en', { month: 'short' }).format(date); // Get short month name
  ...
  const filterData = (timeRange) => {
    ...
    } else if (timeRange === "allTime") { // Add case for all time
      // No filtering needed, return original reviews
      return reviews;
    }
